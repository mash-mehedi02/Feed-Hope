...
// Handle food approval/rejection
    ...
    if($action == 'approve') {
        ...
        if(mysqli_query($connection, $update_query)) {
            ...
            $history_query = "INSERT INTO food_history 
                             ...
                             VALUES ('$donation_id', 'pending', 'available', 'admin', '$admin_id', '$admin_name', '$notes')";
            ...
            $notification_query = "INSERT INTO notifications 
                                 ...
                                 VALUES ('{$donation_data['email']}', '$donation_id', 'status_update', 
                                        'Your food donation \"{$donation_data['name']}\" has been approved and is now available for booking!', 
        ...
        // Update food status to rejected
        $update_query = "UPDATE food_donations SET 
                        status = 'rejected', 
                        admin_approved_by = '$admin_id',
                        admin_approved_at = NOW(),
        ...
        if(mysqli_query($connection, $update_query)) {
            ...
            $history_query = "INSERT INTO food_history 
                             (donation_id, status_from, status_to, changed_by_type, changed_by_id, changed_by_name, notes) 
                             VALUES ('$donation_id', 'pending', 'rejected', 'admin', '$admin_id', '$admin_name', '$notes')";
            mysqli_query($connection, $history_query);
            ...
            $notification_query = "INSERT INTO notifications 
                                 (user_email, donation_id, type, message, status_from, status_to, action_by_type) 
                                 VALUES ('{$donation_data['email']}', '$donation_id', 'status_update', 
                                        'Your food donation \"{$donation_data['name']}\" was not approved. Reason: $notes', 
                                        'pending', 'rejected', 'admin')";
            mysqli_query($connection, $notification_query);
            
            echo '<script>alert("Food donation rejected.");</script>';
        }
    }
}
...
$sql = "SELECT * FROM food_donations WHERE status = 'pending' AND location = '$loc' ORDER BY date DESC";
